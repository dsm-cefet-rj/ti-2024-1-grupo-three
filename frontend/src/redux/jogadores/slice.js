...
const getJogadores = createAsyncThunk('jogadores/getJogadoresAsync', async (data) => {
  try{
        const config = {
            headers: {
...
const getAllJogadores = createAsyncThunk("jogadores/getAllJogadoresAsync", async () => {
    try{
...
const jogadoresSlice = createSlice({
  ...
  reducers: {
    addJogadores: (state, action) => {
      state.inicialState = action.payload;
    }
  }
});
