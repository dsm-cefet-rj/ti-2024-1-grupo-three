...
const Convite = () => {
  ...
  useEffect(() => {
    ...
    const fetchTeamNames = async () => {
      ...
      for (const convite of convitesTime) {
        if (!teamNames[convite.timeRemetente]) {
          try {
            const response = await axios.get(`http://localhost:3004/api/time/${convite.timeRemetente}`);
    ...
    const fetchTorneioNames = async () => {
      ...
      setTorneioNames((prevNamesTorneio) => ({...prevNamesTorneio, ...namesTorneio}));
    }
  ...
  const handleRejeitar = async (idConvite) => {
    ...
    setConvitesTime((prev) => prev.filter((convite) => convite._id !== idConvite));
  };
  ...
  const handleAceitar = async (idConvite) => {
    ...
    const response = await axios.put(`http://localhost:3004/api/convite/aceitar/${idConvite}`);
      ...
      <div className="container-convites">
        ...
        {convitesTime.length > 0 ? (
          convitesTime.map((convite) => (
            <div key={convite._id} className="conteudo-convites">
              <p>time: {teamNames[convite.timeRemetente] || 'Carregando...'}</p>
        ...
        {convitesTorneio.length > 0 ? (
          convitesTorneio.map((convite) => (
            <div key={convite._id} className="conteudo-convites">
              <p>torneio: {torneioNames[convite.torneio] || 'Carregando...'}</p>
              <div className="btn-group">
                <button
                  type="button"
                  className="btn-aceita"
